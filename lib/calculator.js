...
export function calculatePlanEconomics(
  ...
  if (plan.artworkSource === 'custom') {
    // Custom Planning Benchmark Scenario
    ...
    // Actual artwork selection from Curated / Inventory collection
    if (Array.isArray(plan.selectedPaintingIds)) {
      // Plan has its own customized selection of painting IDs
      ...
      // Inherit active curated collection from Step 03 Curate
      ...
      // Default planning scenario assumption fallback (when no artworks exist yet in inventory)
  ...
  // 4. Monthly Direct Client Delivery Costs (Direct monthly service delivery)
  ...
  const manpowerMonthly = Math.max(0, num(plan.manpowerMonthly));
  ...
  // 5. Company Overhead Allocation (Staff + General Overhead)
  const companyOverheadMonthly = Math.max(0, num(plan.companyOverheadMonthly));
  const totalCompanyOverheadAllocation = allocatedEmployeeSalary + companyOverheadMonthly;

  // Total combined monthly operating cost
  const totalMonthlyOperatingCosts = totalMonthlyClientDeliveryCost + totalCompanyOverheadAllocation;

  // 6. Project & Visit-Based Costs (On-Demand / Per-Cycle)
  const curatorFeePerCycle = Math.max(0, num(plan.curator?.feePerCycle, 2000));
  const curatorCycles = Math.max(1, num(plan.curator?.cycles, 1));
  const totalCuratorCost = curatorFeePerCycle * curatorCycles;
  const curatorCalc = {
    feePerCycle: curatorFeePerCycle,
    ...
    justification:
      plan.curator?.justification ||
      `Base fee of ₹${fmtNum(curatorFeePerCycle)} across ${curatorCycles} curation cycle(s) (${spaceSqFt} sq ft, ${artworkCount} artworks)`,
  };
  ...
  const rotationCycles = Math.max(0, num(plan.rotation?.cycles, 1));
  ...
  const packagingCycles = Math.max(1, num(plan.packaging?.cycles, 1));
  ...
  // 7. Subscription Economics & Whiteboard Model
  const monthlySubscription = Math.max(0, num(plan.monthlySubscription));
  
  // Whiteboard Level 1: Monthly Client Contribution (Before Company Overhead)
  const monthlyContributionBeforeOverhead = monthlySubscription - totalMonthlyClientDeliveryCost;

  // Whiteboard Level 2: Contribution After Company Overhead Allocation
  ...
  const clientContributionRecoveryMonths =
    initialInvestment > 0 && monthlyContributionBeforeOverhead > 0
      ? initialInvestment / monthlyContributionBeforeOverhead
      : null;
  ...
  return {
    ...
    initialInvestment,
    // Category B: Monthly Direct Delivery Costs
    maintenanceMonthly,
    artistRecurringMonthly,
    manpowerMonthly,
    travelMonthlyAllocation,
    ...
    // Category C: Company Cost Allocation
    companyEmployeePool: companyCalc.totalEmployeeSalary,
    employeeAllocationType: plan.employeeAllocationType || 'percent',
    ...
    // Category D: Project & Visit-Based Costs
    curator: curatorCalc,
    installation: {
    ...
    // Core Subscription Economics & Whiteboard Model
    monthlySubscription,
    monthlyContributionBeforeOverhead,
    monthlyContribution,
    simpleRecoveryMonths: economics.simpleRecoveryMonths,
    estimatedRecoveryMonths: economics.estimatedRecoveryMonths,
    clientContributionRecoveryMonths,
...
/**
 * Company Sustainability & Portfolio Mix Engine
 * Evaluates company-level revenue, direct client delivery costs,
 * company recurring pool, and net sustainability across a mix of active clients.
 */
export function calculatePortfolioSustainability(
  plans = {},
  portfolioClients = {},
  companyCosts = {},
  plansEconomics = {}
  ...
  const companyRecurringCost =
    ...
    (companyCalc.otherRecurringExpenses || []).reduce((s, o) => s + Math.max(0, num(o.amount)), 0);
  ...
  for (const [planId, plan] of Object.entries(plans)) {
    const clientCount = Math.max(0, parseInt(portfolioClients[planId], 10) || 0);
    ...
    breakdown.push({
      ...
      staffAllocTotal,
      // Single plan breakeven: how many clients of this plan alone to cover entire company pool
      singlePlanBreakevenClients:
        clientCont > 0 ? Math.ceil(companyRecurringCost / clientCont) : null,
    });
  }

  // Portfolio Contribution after company recurring pool
  const portfolioContributionAfterCompanyRecurringCosts =
    totalMonthlyClientContributionBeforeOverhead - companyRecurringCost;

  // Average client contribution in portfolio
  const avgContributionPerClient =
    totalActiveClients > 0
      ? totalMonthlyClientContributionBeforeOverhead / totalActiveClients
      : 0;

  const portfolioBreakevenClients =
    avgContributionPerClient > 0
      ? Math.ceil(companyRecurringCost / avgContributionPerClient)
      : null;
  ...
  return {
    ...
    portfolioBreakevenClients,
    isSustainable: portfolioContributionAfterCompanyRecurringCosts >= 0,
  };
}
