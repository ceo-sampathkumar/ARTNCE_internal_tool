 ...
 * @returns {Object} { monthlySubscription, monthlyOperatingCost, monthlyContribution, contributionPercent, costsBreakdown }
 */
export function calculateSimplePlanResult({ monthlySubscription = 0, costs = [], options = {} }) {
  ...
  const monthlyContribution = sub - monthlyOperatingCost;
 ...
 * 1. TOTAL SPEND: The actual total amount ARTNCE spends for that specific client over the calculation period.
 * 2. AVG. MONTHLY SPEND: Total Spend / periodMonths.
 * 3. CONTRIBUTION: Monthly Fee - Avg Monthly Spend.
 *
 * @param {Object} params
 ...
 * @returns {Object} { totalSpend, avgMonthlySpend, monthlyContribution, contributionPercent, periodMonths, items }
 */
export function calculateClientEconomics({
  ...
  const monthlyContribution = fee - avgMonthlySpend;
...
export function calculateCompanyTargetPerformance({
  ...
  const totalCompanyMonthlyRequirement =
    salaries + rent + travel + tech + marketing + investments + other + customItemsTotal;
